...
window.CRST_STORE = (function () {
  ...
  const DEMO_STRESS_COMPANIES = [
    ...
    { companyName: '冀东钢铁冶炼', branchName: '唐山分行', branchCode: '1302', standardIndustry: '钢铁', gbIndustryCode: 'C3110', customerId: 'CUST-1013', revenueBase: 88000, pdBase: 0.021, loanBalance: 62000, prevStatus: 'NORMAL', defaultTier: 2 },
    { companyName: '辽东特钢制造', branchName: '沈阳分行', branchCode: '2101', standardIndustry: '钢铁', gbIndustryCode: 'C3120', customerId: 'CUST-1014', revenueBase: 76000, pdBase: 0.024, loanBalance: 55000, prevStatus: 'ATTENTION', defaultTier: 2 },
    ...
    { companyName: '赣南铜业集团', branchName: '南昌分行', branchCode: '3601', standardIndustry: '有色', gbIndustryCode: 'C3211', customerId: 'CUST-1015', revenueBase: 105000, pdBase: 0.017, loanBalance: 58000, prevStatus: 'NORMAL', defaultTier: 2 },
    { companyName: '云铝冶炼股份', branchName: '昆明分行', branchCode: '5301', standardIndustry: '有色', gbIndustryCode: 'C3216', customerId: 'CUST-1016', revenueBase: 92000, pdBase: 0.022, loanBalance: 48000, prevStatus: 'NORMAL', defaultTier: 2 },
    ...
    { companyName: '陕北煤业开发', branchName: '西安分行', branchCode: '6101', standardIndustry: '煤炭', gbIndustryCode: 'B0610', customerId: 'CUST-1017', revenueBase: 96000, pdBase: 0.027, loanBalance: 65000, prevStatus: 'NORMAL', defaultTier: 2 },
    { companyName: '蒙东褐煤集团', branchName: '呼和浩特分行', branchCode: '1501', standardIndustry: '煤炭', gbIndustryCode: 'B0690', customerId: 'CUST-1018', revenueBase: 84000, pdBase: 0.029, loanBalance: 52000, prevStatus: 'ATTENTION', defaultTier: 2 },
      ...
      companies.forEach((c, ci) => {
        scenarioList.forEach((sc, si) => {
          ...
          const defaultFlag = (() => {
            ...
            if (sc.code === 'ORDERLY_TRANSITION' && tier >= 1 && y >= 2034) return true;
            if (tier >= 3 && y >= 2030) return true;
            return false;
          })();
          const postStatus = defaultFlag ? 'SUBSTANDARD' : (c.prevStatus || 'NORMAL');
          rows.push({
            companyName: c.companyName,
            branchName: c.branchName,
            standardIndustry: c.standardIndustry,
  ...
  let tasks = [
    {
      ...
      syncFilters: { loanRegion: 'DOMESTIC', loanClasses: [] },
      description: '季度例行压测',
      status: 'COMPLETED',
  ...
  let taskLogs = {
    ...
    2: [
      ...
      { time: '2025-06-04 09:05:00', action: '数据同步与确认：开始同步财务数据（境内贷款，PD按数仓基期值同步）', operator: '总行管理员' },
  ...
  let exportLogs = [
    {
      ...
      exportType: '表格',
      fileFormat: 'Excel',
      scope: '华夏银行压力测试结果汇总表（外报）',
      filter: '场景=全部，年份=全部，维度=行业',
      fields: '行业大类,子行业,指标,2023年（基准）,2024年,...',
      filterSnapshot: { context: 'stressSummary', scenarioCode: 'BASELINE', year: '', scenarioFilter: '', dim: 'industry' },
      fieldKeys: [],
      operator: '总行管理员',
      exportedAt: '2025-06-03 18:45:00',
      downloadFileName: '华夏银行压力测试结果汇总表（外报）.xls',
      moduleName: '压测结果分析',
    },
    {
      id: 2,
      ...
      exportType: '表格',
      fileFormat: 'Excel',
      scope: '违约客户监控 — 行业新增不良/违约户',
      filter: '场景=全部，年份=全部，维度=行业',
      fields: '行业,新增不良/违约户名称,所在分行,预计违约年份',
      filterSnapshot: { context: 'defaultMonitor', year: '', scenarioCode: '', dim: 'industry' },
      fieldKeys: [],
      operator: '总行管理员',
    ...
    },
    {
      ...
      fields: '华夏银行压力测试结果汇总表（外报）.xls, 华夏银行违约调整明细表（外报）.xls',
    ...
    },
    {
      ...
      filterSnapshot: { context: 'analysis', year: '', scenarioCode: '', dim: 'industry' },
    ...
    },
    {
      ...
      fileFormat: 'Excel',
      scope: '华夏银行压力测试结果汇总表（外报）',
      filter: '场景=全部，年份=2040年，维度=行业',
      fields: '行业大类,子行业,指标,2023年（基准）,2024年,...',
      filterSnapshot: { context: 'stressSummary', scenarioCode: 'BASELINE', year: 2040, scenarioFilter: '', dim: 'industry' },
      fieldKeys: [],
      operator: '总行管理员',
